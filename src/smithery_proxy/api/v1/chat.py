...
SMITHERY_MODEL_REFRESH_TTL = 300  # seconds

_BASE_MODEL_DEFINITIONS: tuple[Dict[str, Any], ...] = (
    {
        "id": "claude-haiku-4.5",
        "object": "model",
        "created": 1677610602,
    ...
    },
    {
        ...
        "owned_by": "xai",
        "permission": [],
        "root": "grok-4.1-fast-non-reasoning",
        "parent": None
    },
    {
    ...
    },
    {
        ...
        "owned_by": "deepseek",
        "permission": [],
        "root": "deepseek-v3.2-thinking",
        "parent": None
    },
)

_smithery_models_cache: Dict[str, Any] = {"models": [], "expires_at": 0.0}
_smithery_models_lock = asyncio.Lock()


def _get_static_models() -> List[Dict[str, Any]]:
    return [dict(model) for model in _BASE_MODEL_DEFINITIONS]
...
def _normalize_remote_model_entry(entry: Any) -> Optional[Dict[str, Any]]:
    ...
    permission = entry.get("permission") or entry.get("permissions") or []
    ...
    return {
        ...
        "created": created,
        "owned_by": entry.get("owned_by") or entry.get("owner") or "smithery",
        "permission": permission,
        "root": entry.get("root", model_id),
        "parent": entry.get("parent"),
    }


async def _fetch_smithery_models(settings: Settings) -> List[Dict[str, Any]]:
    if not settings.smithery_auth_token:
        logger.debug("Smithery token 未配置，跳过远程模型同步")
        return []

    url = settings.smithery_url.rstrip("/") + "/api/models"
    headers = {
    ...
    try:
        ...
        logger.warning(f"获取Smithery模型列表失败: {exc}")
        return []

    if response.status_code != 200:
        logger.warning(f"Smithery模型接口返回 {response.status_code}: {response.text[:200]}")
        return []

    try:
        payload = response.json()
    except json.JSONDecodeError as exc:
        logger.warning(f"解析Smithery模型响应失败: {exc}")
        return []

    entries = _extract_model_entries(payload)
    normalized: List[Dict[str, Any]] = []
    for entry in entries:
        normalized_entry = _normalize_remote_model_entry(entry)
        if normalized_entry:
            normalized.append(normalized_entry)

    if normalized:
        logger.info(f"自动同步 {len(normalized)} 个Smithery模型")
    return normalized


async def _get_cached_smithery_models(settings: Settings) -> List[Dict[str, Any]]:
    now = time.time()
    if now < _smithery_models_cache["expires_at"]:
        return _smithery_models_cache["models"]

    async with _smithery_models_lock:
        if now < _smithery_models_cache["expires_at"]:
            return _smithery_models_cache["models"]

        models = await _fetch_smithery_models(settings)
        ttl = SMITHERY_MODEL_REFRESH_TTL if models else 60
        _smithery_models_cache["models"] = models
        _smithery_models_cache["expires_at"] = now + ttl
        return models
...
📄 **Native Document Processing**: I can directly read and analyze various document formats including:
- Text files (.txt)
- Markdown documents (.md)
- CSV data files (.csv)
- PDF documents (.pdf)
I can extract information, summarize content, and answer questions about documents directly.

🛠️ **Additional Tools**: When users need capabilities beyond image and document analysis, I have access to tools including web search, code execution, web content fetching, document management, and data analysis.

**Important**: For image and document analysis tasks, I use my native capabilities directly. I only use external tools when users specifically request web search, code execution, or other specialized tasks."""

def ensure_system_prompt(messages: List) -> List:
    """确保消息列表包含正确的系统提示词，但只在有图片或文档时添加能力描述"""
    ...
    for msg in messages:
        if hasattr(msg, 'content') and isinstance(msg.content, list):
            for item in msg.content:
                if isinstance(item, dict) and item.get('type') in ['image_url', 'document_url', 'file']:
                    has_media = True
                    break
        if has_media:
            break

    # 如果没有媒体内容，保持原有的系统提示词不变
    if not has_media:
...
async def create_chat_completion(
    ...
    # 注意：空列表 [] 也算包含tools参数，表示客户端期望标准OpenAI格式
    original_has_tools = request.tools is not None
    ...
    if settings.tools_enabled:
        ...
        if has_cursor_context:
            ...
            # 优化的工具关键词检测逻辑
            has_tool_keywords = _detect_tool_requirements(messages_text)
            logger.info(f"🔍 智能检测工具需求: {has_tool_keywords}")

        # 决定是否需要添加工具
        # 如果 request.tools 是空列表 []，表示客户端请求自动添加所有工具
        # 如果 request.tools 是 None 或不存在，则根据关键词判断是否需要工具
        # 如果 request.tools 有实际内容，则使用客户端提供的工具列表
        if request.tools is None or request.tools == []:
            # 空列表或None：需要检查是否添加工具
            if request.tools == []:
                # 空列表表示明确请求所有工具
                should_add_tools = True
                logger.info(f"🔧 请求包含空工具列表，将自动添加所有可用工具")
            elif has_tool_keywords:
                # 根据关键词检测判断
                should_add_tools = True
                logger.info(f"🔧 请求无工具定义，但检测到需要工具，将自动添加")
        elif len(request.tools) > 0:
            # 如果请求已有具体的工具定义，记录但不重复添加
            logger.info(f"🔧 请求已包含工具定义，工具数量: {len(request.tools)}")

        if has_image_content:
            logger.info(f"🖼️ 检测到图片内容，将直接通过 experimental_attachments 发送给 Claude（不使用工具）")
    ...
    try:
        ...
        logger.info(f"🔍 [主函数] stream={request.stream}, tool_manager={tool_manager is not None}")
        if request.stream:
            # 流式响应
            logger.info(f"🔍 [主函数] 进入流式响应分支")
            return await _handle_stream_response(
                mcp_client, request, tool_manager, original_user_system_prompt, auth_token
            )
        else:
            # 非流式响应
            logger.info(f"🔍 [主函数] 进入非流式响应分支，准备调用_handle_normal_response")
        ...
        if status_code == 403:
            detail = "当前账号无权访问所选模型，请更换模型或联系管理员。"
        else:
            detail = str(e)
...
async def _handle_normal_response(
    ...
    """处理非流式响应"""
    logger.info(f"🔍 [_handle_normal_response] 函数开始执行！")
    logger.info(f"🔍 [_handle_normal_response] tool_manager={tool_manager is not None}, original_has_tools={original_has_tools}")

    # 检查是否需要工具调用 - 修复逻辑
    # 不仅检查工具定义，还要检查是否真的需要执行工具
    logger.info(f"🔍 [调试] tool_manager存在: {tool_manager is not None}")
    logger.info(f"🔍 [调试] 消息数量: {len(openai_request.messages) if openai_request.messages else 0}")
    if tool_manager:
        # 检查用户消息是否包含需要工具的关键词
        last_message = openai_request.messages[-1] if openai_request.messages else None
        logger.info(f"🔍 [调试] 最后一条消息: role={last_message.role if last_message else None}, content类型={type(last_message.content) if last_message else None}")
                ...
                if last_message and last_message.role == "user":
                    ...
                    if isinstance(user_content, list):
                        ...
                        if has_cursor_context:
                            ...
                            # 如果没有成功创建工具调用，回退到正常AI响应
                            if not tool_calls:
                                logger.info("🔄 没有创建工具调用，回退到正常AI响应")
                                should_handle_tools = False
                            ...
                            if gemini_analysis:
                                ...
                                # 使用之前保存的原始用户系统提示词
                                user_role_prompt = original_user_system_prompt if original_user_system_prompt else ""

                                logger.info(f"🔍 使用保存的原始用户系统提示词")
                                logger.info(f"🔍 最终角色设定: {user_role_prompt if user_role_prompt else '无自定义角色'}")

                                # 构建自然的图片分析提示
                                claude_user_message = f"""用户请求：{user_text.strip()}

图片分析结果：
{img_info}

请基于以上图片分析，用简洁清晰的语言回答用户的问题。"""
...
async def list_models(
    ...
    try:
        remote_models = await _get_cached_smithery_models(_settings)
    except Exception as exc:
        logger.warning(f"同步Smithery模型失败: {exc}")

    if remote_models:
        combined: Dict[str, Dict[str, Any]] = {model["id"]: model for model in models}
        for remote_model in remote_models:
            combined[remote_model["id"]] = remote_model
...
async def _handle_tool_calling_response(
    ...
    if last_message and last_message.role == "user":
        ...
        if should_auto_create_tools:
            ...
            # 如果没有创建工具调用（如图片内容直接发送给Claude），直接调用AI处理
            if not tool_calls:
                logger.info("🖼️ 没有工具调用，图片将直接由Claude处理（通过experimental_attachments）")
                ...
                try:
                    ...
                    logger.info(f"✅ Claude直接处理完成，回复长度: {len(response_content)} 字符")
                except Exception as e:
                    logger.error(f"❌ Claude处理失败: {e}")
                ...
                # 对所有工具结果进行Claude二次处理
                if tool_results:
                    # 所有工具结果都通过Claude处理，生成自然的中文回答
                    if gemini_analysis:
                        logger.info("🤖 开始 Claude 二次处理，结合图片分析生成最终回答")
                    elif has_search_results:
                        logger.info("🤖 开始 Claude 二次处理，结合搜索结果生成最终回答")
                    else:
                        logger.info("🤖 开始 Claude 二次处理，将工具结果转换为自然语言")
                    ...
                    if gemini_analysis:
                        # 图片分析：直接基于分析结果回答
                        claude_user_message = f"""用户问题：{user_text.strip()}

图片分析：
{analysis_info}

请基于以上分析，简洁清晰地回答用户的问题。"""
                    elif has_search_results:
                        # 搜索结果：需要整理和总结
                        claude_user_message = f"""用户问题：{user_text.strip()}
...
请基于以上搜索结果，用自然流畅的中文为用户提供详细且有帮助的回答。"""
                    else:
                        # 其他工具：将执行结果转换为自然语言
                        claude_user_message = f"""用户问题：{user_text.strip()}
...
请基于以上工具执行结果，用简洁自然的中文回答用户的问题。直接给出答案，不需要解释工具的使用过程。"""
                    ...
                    try:
                        ...
                        logger.error(f"❌ Claude 二次处理失败: {e}")
                        # 如果 Claude 处理失败，返回原始工具结果
                    formatted_results = []
                    for result in tool_results:
                        formatted_results.append(f"🔧 **{result.name}**\n\n{result.content}")
                        response_content = "\n\n".join(formatted_results)
                        logger.info(f"⚠️ Claude处理失败，返回原始工具结果")
                else:
        ...
        if original_has_tools:
            # 标准OpenAI工具调用格式 - 如果有tool_calls就必须返回
            if tool_calls:
                logger.info("📋 使用标准OpenAI格式：返回tool_calls（工具已自动执行）")
                choice = ChatCompletionChoice(
                    index=0,
                    message=ChatMessage(
                        role="assistant",
                ...
                logger.info("📋 标准格式设置：finish_reason=tool_calls")
            else:
                # 没有工具调用，直接返回AI回复
                logger.info("📋 无工具调用，返回普通回复")
                choice = ChatCompletionChoice(
                    index=0,
                    message=ChatMessage(
                        role="assistant",
                ...
                logger.info("📋 标准格式设置：finish_reason=stop")
        else:
...
def _create_tool_calls_from_multimodal_request(message, tool_manager: ToolManager) -> List[ToolCall]:
    ...
    # 提取文本内容
    text_content = ""
    has_media = False  # 检测图片或文档附件

    for item in content:
        if isinstance(item, dict):
            if item.get('type') == 'text':
                text_content += item.get('text', '') + " "
            elif item.get('type') in ['image_url', 'image', 'document_url', 'file']:
                # 图片、文档等媒体类型都通过 experimental_attachments 处理
                has_media = True
    ...
    # 如果包含媒体内容（图片/文档），不创建工具调用，让 Claude 通过 experimental_attachments 直接处理
    if has_media:
        logger.info("🖼️📄 检测到多模态内容（图片/文档），将通过 experimental_attachments 直接发送给 Claude（不创建工具调用）")
        # 直接返回空列表，不创建任何工具调用
        return []

    # 如果没有媒体内容但有文本，使用原有的文本分析逻辑
    if not has_media and text_content:
...
def _create_tool_calls_from_request(user_content: str, tool_manager: ToolManager) -> List[ToolCall]:
    ...
    if tool_manager._has_image_content(user_content):
        logger.info("🖼️ 检测到图片内容，将通过 experimental_attachments 直接发送给 Claude（不创建工具调用）")
        # 图片内容不创建工具调用，直接返回空列表
        return []
...
for i, num in enumerate(result):
    print(f"第{i+1}项: {num}")"""
                ...
                # 首先尝试直接提取数学表达式（如 "123 + 456"）
                math_expr_pattern = r'([0-9+\-*/().\s]+(?:[+\-*/][0-9+\-*/().\s]+)+)'
                math_match = re.search(math_expr_pattern, user_content)
                
                if math_match and any(op in user_content for op in ['+', '-', '*', '/']):
                    # 找到数学表达式，生成Python代码
                    ...
                    for pattern in simple_code_patterns:
                        ...
                        if match:
                            ...
                            code = code.strip('"\'')
                            # 移除结尾的中文说明（如"等于多少"）
                            code = re.sub(r'\s*(等于|是|结果|输出).*$', '', code)
    ...
    # 检查数据分析请求（web_fetch 检测在后面有更完整的实现）
    ...
    # 动态检查AstrBot工具调用 - 完全基于检测到的工具和关键词
    def check_astrbot_tool_match(content_lower: str, tool_manager) -> tuple:
        """检查用户内容是否匹配任何AstrBot工具的关键词"""
        if not hasattr(tool_manager, 'detected_astrbot_tools'):
            return None, None
        ...
        for tool_info in detected_tools:
            ...
            matched_keywords = [kw for kw in trigger_keywords if kw.lower() in content_lower]

            if matched_keywords:
                logger.info(f"🎯 动态匹配到AstrBot工具: {tool_name}, 匹配关键词: {matched_keywords}")
                return tool_name, tool_info

        return None, None

    # 使用动态检测替代硬编码
    matched_tool_name, matched_tool_info = check_astrbot_tool_match(content_lower, tool_manager)

    if matched_tool_name:
        logger.info(f"🎯 检测到AstrBot工具调用请求: {matched_tool_name}")

        # 动态创建AstrBot工具调用
        try:
            # 使用通用参数提取
            capabilities = matched_tool_info.get('capabilities', {})
            params = _extract_universal_tool_parameters(matched_tool_name, user_content, capabilities)

            # 特殊处理图片生成工具
            if matched_tool_name == "gemini_draw":
                # 提取图片描述
                prompt = user_content
                # 清理用户ID信息
                ...
                # 移除生成相关的词汇，保留描述
                trigger_keywords = capabilities.get('trigger_keywords', [])
                for word in trigger_keywords + ["一张", "图片", "图像", "照片"]:
            ...
            if params:
                ...
                tool_calls.append(tool_call)
                logger.info(f"✅ 成功创建AstrBot工具调用: {matched_tool_name}")
            else:
                logger.warning(f"⚠️ 无法提取工具参数: {matched_tool_name}")

        except Exception as e:
            logger.error(f"❌ 创建AstrBot工具调用失败 {matched_tool_name}: {e}")
    ...
    # 检查文档管理请求
    doc_keywords = ["创建文件", "保存文件", "写入文件", "create file", "save file", "write file"]
    if any(keyword in content_lower for keyword in doc_keywords):
        logger.info("📁 检测到文档管理请求")
        try:
            # 提取文件名
            filename_patterns = [
                r'文件[名]?\s*[：:]\s*([^\s,，]+)',
                r'名为\s*([^\s,，]+)',
                r'([a-zA-Z0-9_\-]+\.[a-zA-Z0-9]+)',  # test.txt 格式
            ]
            ...
            for pattern in content_patterns:
                match = re.search(pattern, user_content, re.IGNORECASE)
                if match:
                    file_content = match.group(1).strip().strip('"\'')
                    break
            
            logger.info(f"📁 提取的文件名: {filename}, 内容: {file_content[:50]}...")
            
            # 创建 document_manager 工具调用（title使用filename）
            tool_call = tool_manager.create_tool_call("document_manager", {
                "action": "create",
                "title": filename,  # 添加必需的title参数
                "content": file_content
            })
            tool_calls.append(tool_call)
            logger.info(f"✅ 成功创建文档管理工具调用")
            
        except Exception as e:
            logger.error(f"❌ 创建文档管理工具调用失败: {e}")
...
def _auto_extract_keywords_from_description(description: str) -> list:
    """从工具描述中自动提取关键词，优先解析AstrBot自带的关键词"""
    keywords = []

    # 首先尝试解析AstrBot工具描述中明确提到的关键词
    # 格式：调用关键词"生成"、"图像处理"、"画"等
    keyword_patterns = [
        r'调用关键词["""]([^"""]+)["""]',  # 调用关键词"xxx"
        r'关键词["""]([^"""]+)["""]',      # 关键词"xxx"
        r'keywords?["""]([^"""]+)["""]',   # keywords"xxx"
        r'trigger["""]([^"""]+)["""]',     # trigger"xxx"
    ]
    ...
    for pattern in keyword_patterns:
        ...
        for match in matches:
            # 解析引号内的关键词，支持中英文分隔符
            explicit_keywords = re.split(r'[、，,;；\s]+', match.strip())
            keywords.extend([kw.strip().strip('"\'') for kw in explicit_keywords if kw.strip()])

    # 如果找到了明确的关键词，优先使用这些
    if keywords:
        logger.info(f"🎯 从工具描述中解析到明确关键词: {keywords}")
        return list(set(keywords))[:15]  # 返回去重后的关键词
...
def _detect_tool_requirements(messages_text: str) -> bool:
    """
    智能检测是否需要工具调用
    区分代码生成请求和工具执行请求
    """
    ...
    # 1. 明确的搜索请求 - 需要工具
    search_patterns = [
        "搜索", "search", "查找", "find", "寻找", "查询",
        "搜一下", "找一下", "查一下"
    ]
    if any(pattern in text_lower for pattern in search_patterns):
        logger.info("🔍 检测到搜索请求，需要工具")
        return True

    # 2. 网页获取请求 - 需要工具
    web_fetch_patterns = [
        "获取网页", "抓取网页", "访问网站", "打开网址", "fetch", "crawl"
    ]
    if any(pattern in text_lower for pattern in web_fetch_patterns):
        logger.info("🌐 检测到网页获取请求，需要工具")
        return True

    # 3. 代码执行请求 - 需要工具
    code_execution_patterns = [
        "执行代码", "运行代码", "测试代码", "run code", "execute",
        "运行这个", "执行这个", "测试这个"
    ]
    if any(pattern in text_lower for pattern in code_execution_patterns):
        logger.info("⚡ 检测到代码执行请求，需要工具")
        return True

    # 4. 数据分析请求 - 需要工具
    data_analysis_patterns = [
        "分析数据", "统计", "计算平均", "数据处理", "analyze data",
        "最高", "最低", "平均值", "总和", "求和"
    ]
    if any(pattern in text_lower for pattern in data_analysis_patterns):
        logger.info("📊 检测到数据分析请求，需要工具")
        return True

    # 5. 文件操作请求 - 需要工具
    file_operation_patterns = [
        "保存文件", "创建文件", "读取文件", "save file", "create file",
        "写入文件", "导出", "下载文件"
    ]
    if any(pattern in text_lower for pattern in file_operation_patterns):
        logger.info("📁 检测到文件操作请求，需要工具")
        return True

    # 6. 代码生成请求 - 不需要工具（直接AI响应）
    code_generation_patterns = [
        "写代码", "生成代码", "创建代码", "write code", "generate code",
        "写一个", "创建一个", "生成一个", "帮我写",
        "写个函数", "写个类", "写个脚本",
        "写网页", "创建网页", "生成网页", "做个网站"
    ]
    if any(pattern in text_lower for pattern in code_generation_patterns):
        logger.info("💻 检测到代码生成请求，使用AI直接响应")
        return False

    # 7. 一般性问答 - 不需要工具
    general_qa_patterns = [
        "什么是", "如何", "怎么", "为什么", "解释", "介绍",
        "what is", "how to", "why", "explain", "tell me"
    ]
    if any(pattern in text_lower for pattern in general_qa_patterns):
        logger.info("💬 检测到一般问答，使用AI直接响应")
        return False

    # 8. 默认情况：如果包含可能的工具关键词但不明确，倾向于不使用工具
    ambiguous_keywords = ["代码", "code", "网页", "webpage", "创建", "create"]
    if any(keyword in text_lower for keyword in ambiguous_keywords):
        logger.info("🤔 检测到模糊关键词，倾向于AI直接响应")
        return False

    # 9. 其他情况默认不需要工具
    logger.info("📝 默认使用AI直接响应")
    return False
